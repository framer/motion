...
import { TargetResolver, VariantLabels } from "../motion/types"
 ...
 * SVG path overrides
 *
 * { delay: 1, pathLength: { delay: 0.4 }}
 */
export type SVGPathTransitionOverrides = {
    [K in keyof SVGPathProperties]?: Transition
}

export type CustomTransitionOverrides = {
    [K in keyof CustomStyles]?: Transition
}
...
export type VariantTransition = TransitionWithOverrides & VariantOrchestration
...
export interface AnimationOptions<V = any>
    extends Omit<TransitionWithPlaybackLifecycles<V>, "type"> {
    keyframes: V[]
    type?: TransitionType
}
...
export type ResolvedKeyframes =
    | GenericKeyframes<number>
    | GenericKeyframes<string>

export type ValueKeyframe = string | number

export type ResolvedValue = ValueKeyframe | ResolvedKeyframes

export type UnresolvedValueKeyframe = ValueKeyframe | null

export type StyleValuesDefinition = {
    [K in keyof CSSStyleDeclarationWithTransform]?: ValueKeyframe
}

export type SVGPathValuesDefinition = {
    [K in keyof SVGPathProperties]?: ValueKeyframe
}

export type VariableValuesDefinition = {
    [key: `--${string}`]: ValueKeyframe
}

export type CustomValuesOverrides = {
    [K in keyof CustomStyles]?: ValueKeyframe
}

export type DOMValuesDefinition = StyleValuesDefinition &
    SVGPathValuesDefinition &
    CustomValuesOverrides &
    VariableValuesDefinition
...
export type CustomKeyframesOverrides = {
    [K in keyof CustomStyles]?: ValueKeyframesDefinition
}
...
/**
 * This is copy/pasted from @types/react-dom
 */
