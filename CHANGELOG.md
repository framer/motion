...
-   Fixed `layout` prop crashing Motion One 3D.
...
-   Variant `transition` fixed in Motion One 3D.
...
-   Fixing mapping Motion One easing names to WAAPI.
...
-   Will display a warning if different versions of Motion One are mixed.
...
-   Added `initial` prop to Motion One 3D types.
...
-   Sharing `MotionContext` across Motion One and Motion One 3D environments in CommonJS environments.
...
-   Fixing drag in Android devices by adding aggressive viewport scroll blocking. This will need dialling back down when it comes to open sourcing Motion One so, for instance, a horizontal carousel doesn't block vertical scrolling.
